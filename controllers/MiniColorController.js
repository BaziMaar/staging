const LuckyTransaction = require("../models/MiniColorTransactionsModel.js");
const User = require("../models/userModel.js");  // Import the User model
const Ref=require('../models/referModel')
const Result=require('../models/MiniResultModel.js')
const ColorEntryTransaction=require('../models/MiniColorTransactionsEntryModel.js')
...
const generateAndBroadcastNumber = async(io) => {
  ...
  const generateAndBroadcast = () => {
    ...
    intervalId = setInterval(async() => {
      if (timeRemaining > 5) {
        timeRemaining--;
        io.emit('minicolorPlaced',{voilet:firstBet,green:secondBet,red:thirdBet,zero:allBet[0],one:allBet[1],two:allBet[2],three:allBet[3],four:allBet[4],five:allBet[5],six:allBet[6],seven:allBet[7],eight:allBet[8],nine:allBet[9],small:smallSizeBet,big:bigSizeBet})
        io.emit('minicolorBet', { number: currentNumber, time: timeRemaining,spin:spin, result: winner,globalNumbers:finalNumber,a:lastNumbers[0],b:lastNumbers[1],c:lastNumbers[2],d:lastNumbers[3],e:lastNumbers[4],f:lastNumbers[5],g:lastNumbers[6],h:lastNumbers[7],i:lastNumbers[8],j:lastNumbers[9],k:lastNumbers[10],l:lastNumbers[11]}); 
      }
...
const sendColorMoney = async (io, phone, color, number, size, amount,globalNumber) => {
  try {
    ...
    if (sender.wallet < amount) {
      io.emit('miniwalletColorUpdated', { phone: phone, error: 'Insufficient Funds' });
    ...
    await sender.save();
    io.emit('miniwalletColorUpdated', { phone, newBalance: sender.wallet, color, size, number,globalNumber });

    return { success: true, message: 'Money sent successfully', newBalance: sender.wallet, color, size, number,globalNumber };
    
  } catch (error) {
    io.emit('miniwalletColorUpdated', { error: error.message });
...
const receiveMoney = async (io, phone, color, number, size, amount, globalNumber) => {
  ...
  try {
    ...
    io.emit('miniwalletColorUpdated', {
      phone,
